...
const sections = [
  ...
  },
  {
    ...
    features: [
      ...
      "スタッフマスタ: 従業員情報の登録・編集・削除、プロフィール画像のアップロード",
    ],
  },
];
...
const tips = [
  ...
  },
  {
    ...
    title: "マスタデータの登録順序",
    description:
      "従業員を登録する前に、先に「部署マスタ」と「スキルマスタ」を登録しておくと、従業員登録時に部署やスキルを紐づけできます。",
  },
  {
    icon: Users,
    title: "テーブルの並び替え",
    description:
      "スタッフマスタのテーブルでは、列ヘッダーをクリックして昇順・降順のソートができます。もう一度クリックするとソート解除になります。",
  },
];
...
const stepGuides = [
  ...
  },
  {
    title: "部署を登録・管理する",
    steps: [
      "サイドバーの「マスタ管理」>「部署マスタ」をクリック",
      "「新規追加」ボタンで部署を追加",
      "部署名・部門長・予算・カラーコードを入力",
      "「保存」で登録完了",
      "登録した部署はスタッフマスタの部署選択に自動的に反映される",
    ],
  },
  {
    title: "スキルカテゴリとスキルを設定する",
    steps: [
      "サイドバーの「マスタ管理」>「スキルマスタ」をクリック",
...
const faqItems = [
  {
    ...
    a: "マスタ管理で登録した従業員は、Supabase経由で全ページに自動反映されます。ページをリロードしても表示されない場合は、ブラウザのキャッシュをクリア（Ctrl+Shift+R）してみてください。Supabaseが未設定の場合はモックデータが表示されるため、マスタで追加した従業員は表示されません。",
  },
  {
    q: "グレード体系はどうなっていますか？",
  ...
  },
  {
    ...
    a: "「在籍（active）」「休職中（onLeave）」「リモート（remote）」の3種類です。従業員一覧やスタッフマスタでステータスのフィルタリング・ソートが可能です。休職者がいる場合はダッシュボードにアラートが表示されます。",
  },
  {
...
const troubleshooting = [
  ...
  {
    symptom: "マスタ管理ページでデータが表示されない（読み込み中のまま）",
    solutions: [
        ...
        <div className="p-6 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 space-y-4">
          <div>
            ...
            <p className="text-sm text-gray-700 dark:text-gray-300">
              すべてのマスタデータはSupabase（PostgreSQL）に保存されます。マスタ管理で登録したデータは、ダッシュボード・従業員管理・部署管理・スキルマトリクス・戦略インサイトの各ページにリアルタイムで反映されます。
            </p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-sm text-gray-700 dark:text-gray-300">
              Supabaseが設定済みの場合は実データ、未設定の場合はサンプルのモックデータが表示されます。モックデータ表示中はマスタ管理での変更が他のページに反映されません。
            </p>
          </div>
        </div>
      </section>
